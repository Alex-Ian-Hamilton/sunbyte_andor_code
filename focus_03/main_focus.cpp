...
#include <sstream>
#define imagesample 4 //cant be 1 ,logical errer will occur
    ...
    if (i_available)
    {
      ...
      {      
        ...
        if (errorOk(i_err, "AT_GetIntMax 'AOI Height'\n"))
        {
          ...
          if (errorOk(i_err, "AT_SetInt 'AOI Height'\n") && b_verbose)
          {
    ...
    if (i_available) {
      ...
      {          
        ...
        if (errorOk(i_err, "AT_GetintMax 'AOI Width'\n"))
        {
          ...
          if (errorOk(i_err, "AT_SetInt 'AOI Width'\n") && b_verbose)
          {
    ...
    if (i_available)
    {
      ...
      {  
        ...
        if (b_set16bit)
        {
          ...
          if (errorOk(i_err, "AT_SetEnumString 'PixelEncoding'\n") && b_verbose)
          {
 ...
 *         Code to get some stats on the current image.
			 calulate the average pixel value, max, min and contrast,
	 		returning contrast value*/

int collectStats(unsigned char * _puc_image, AT_64 _i64_width, AT_64 _i64_height, AT_64 _i64_stride)
{ int contrast1 = 0;
	/* a function that calulate 
	the average pixel value, max, min and contrast,
	 returning contrast value*/
  ...
  for (AT_64 jj = 0; jj < _i64_height; jj++)
  {
    ...
    { 
    /* doing a loop over all the pixel
	comparing the current pixel value,
	if the value is largest among all assign it to variable "i64_max" 
	if smallest assign to "i64_min
	Also calculating the average pixel value"*/
  ...
  if (b_verbose)
  {
    ...
    contrast1 = i64_max-i64_min;
printf("getting value of contrast %d",contrast1);
  }      
  ...
  if (errorOk(i_err, "AT_QueueBuffer\n"))
  {
    ...
    if (errorOk(i_err, "AT_Command 'Acquisition Start'\n"))
    {
      ...
      if (errorOk(i_err, "AT_WaitBuffer\n"))
      {
        ...
        {
          contrast = collectStats(puc_returnBuf, i64_aoiWidth, i64_aoiHeight, i64_aoiStride);
          if (i_minScale < 0)
          {
            i_minScale = i64_min;
            i_maxScale = i64_max;
          ...
          if (mode == 1)
          {
            ...
            sprintf(sz_filename_full, "%s_%ld_%ld.%s", sz_filename, tv.tv_sec, tv.tv_usec,sz_fileextension);
...
int main(int argc, char ** argv)
{
  ...
  if (i_err == 0)
  {
    ...
    focusScope(1, 4); // need to add a return if focusing is error
